...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
    ...
    public ArrayList<Location> getPerimeter() {
        ArrayList<Location> perms = new ArrayList<>();
        for (Block block : getBlocks()) {
            if (block.getX() == this.x1 || block.getX() == this.x2 || block.getZ() == this.z1 || block.getZ() == this.z2) {
                perms.add(block.getLocation());
            }
        }
        return perms;
    }
